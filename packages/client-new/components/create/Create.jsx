...
import Charts from "../pages/collection/Charts";
import useSilverSurfer from "@/hooks/useSilverSurfer";
  ...
  return (
    <div className="mt-2 text-white text-sm">
      <div className="grid grid-cols-2">
        <div>
          Balance : {balances && Number(BigNumber(balances.usds).dividedBy(BigNumber(10 ** 9)))} USD-SilverSurfer
        </div>
        <div className="text-right">
          Available: {balances && Number(BigNumber(balances.usdc).dividedBy(BigNumber(10 ** 9)))} USDC
        </div>
      </div>
    </div>
  )
...
export default function Create() {
  ...
  const { getTokenAccounts } = useSilverSurfer()

  const [balances, setBalances] = useState()

  useEffect(() => {
    publicKey && getTokenAccounts(publicKey).then(setBalances)
  }, [publicKey])

  const [tab, setTab] = useState(TAB.MINT)

  const { test, mint } = useSilverSurfer()

  const [amount, setAmount] = useState(1)

  const onMint = useCallback(async () => {

    alert("Not available on the frontend. Please run a script on Solana Playground.")
  ...
  return (
    <section className="relative py-24">
      ...
      <div className="container">
        <h1 className="py-8 text-center font-display text-4xl font-medium text-jacarta-700 dark:text-white">
          Mint USD-SilverSurfer
        </h1>
            ...
            <div className="mx-auto bg-jacarta-700 p-4 py-8 pt-2 rounded-lg">
              ...
              <div class="text-sm font-medium text-center text-gray-500 border-b border-gray-200 dark:text-gray-400 dark:border-gray-700">
                <ul class="flex flex-wrap -mb-px">
                  <li class="me-2">
                    <div onClick={() => setTab(TAB.MINT)} class={`cursor-pointer inline-block p-4 border-b-2 border-transparent rounded-t-lg hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300 ${tab === TAB.MINT && "text-white border-white"}`}>Mint</div>
                  </li>
                  <li class="me-2">
                    <div onClick={() => setTab(TAB.WITHDRAW)} class={`cursor-pointer  inline-block p-4 border-b-2 border-transparent rounded-t-lg hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300  ${tab === TAB.WITHDRAW && "text-white border-white"}`}>Withdraw</div>
                  </li>
              ...
              {tab === TAB.MINT && (
                <div className="mb-6 mt-4">
                  ...
                  <div className="mt-4">
                    <button
                      onClick={onMint}
                      className="cursor-pointer w-full rounded-full bg-accent py-3 px-8 text-center font-semibold text-white transition-all"
                    >
                      Mint
                    </button>
                  </div>
...
//                     <span
//                       className="inline-block"
//                       data-tippy-content="Setting your asset as explicit and sensitive content, like pornography and other not safe for work (NSFW) content, will protect users with safe search while browsing SilverSurfer."
//                     >
//                       <svg
//                         xmlns="http://www.w3.org/2000/svg"
//                         viewBox="0 0 24 24"
